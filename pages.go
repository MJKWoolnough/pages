...
func NewString(baseTemplate string) (*Pages, error) {
	defaultTemplate, err := template.New("").Parse(baseTemplate)
	if err != nil {
...
func (p *Pages) Rebuild() error {
	...
	if p.isFile {
		if np, err = New(p.baseTemplate); err != nil {
			return fmt.Errorf("error reloading templates: %w", err)
		}
	} else {
		np = &Pages{defaultTemplate: p.defaultTemplate, templates: make(map[string]dataFile)}
	}
